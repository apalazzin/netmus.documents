...
\rowcolor{orange} \bo{Metodo} & \bo{Descrizione} \\
\hline
 - user: String \emph{@Key} & \\\hline
 - password: String & Contiene l'hash della password dell'utente
 ottenuto utilizzando l'algoritmo BCrypt.\\\hline 
 - nickName: String & \\\hline 
 - firstName: String & \\\hline
 - lastName: String & \\\hline
 - gender: char & \\\hline
 - nationality: String & \\\hline
 - avatar: Image \emph{@Type(Blob.class)} & \\\hline
 - aboutMe: String \emph{@Type(Text.class)} & \\\hline
 - birthDate: Date & \\\hline
 - registrationDate: Date & \\\hline
 - lastLogin: Date & \\\hline
 - lastImport: Date & \\\hline
 - musicLibrary: MusicLibrary \emph{@Child} & \\\hline
\end{longtable}
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Metodo} & \bo{Descrizione} \\
\hline
+ findUser(in String): UserAccount & \\\hline
+ toUserSummaryDTO(): UserSummaryDTO & \\\hline
+ toUserDTO(): UserDTO & \\\hline
+ toUserCompleteDTO(): UserCompleteDTO & \\\hline
+ getters() & \\\hline
+ setters() & \\\hline
\end{longtable}
