...
\emph{persistent} con inserimento della sottosezione ``Business login
interna''.\\\hline
...
\\\hline 
13/02/2011 & 0.8 & Palazzin Alberto & Aggiunto il package
\emph{server.youtube}.\\
\hline
...
\begin{itemize}
  ...
  per la lista degli album disponibili da cui gli altri oggetti possono
  attingere. La business logic associata \`e gestita all'interno della classe.
  ...
  gestione, anche se non rappresenta la pi\`u efficiente in assoluto, migliora
  in modo sensibile i tempi di esecuzione delle operazioni di load/store nel
...
\caption{Diagramma UML di sequenza che descrive nel dettaglio come viene
...
conferma che propone errori e messaggi. \\\hline
...
mode. \\\hline
@UiField switch\_list: Image & Rappresenta il bottone per visualizzare il
...
partire il lettore flash YouTube\\\hline
+ sortCatalogo() & Utilizzato per ordinare la lista che rappresenta il
catalogo dell'utente.\\\hline
+ showCatalogo() & Utilizzato per caricare e mostrare il catalogo
dell'utente.\\\hline
...
\co{ProfileActivity} che necessitano di intervenire sia in scrittura che in
lettura sul Datastore. Infatti, oltre a salvare il voto assegnato dall'utente,
deve ricevere la nuova media dei voti della canzone calcolata dal server. La
...
avviene la chiamata RPC a \co{SongService} per la scrittura in memoria del voto
...
dati riguardanti la pagina delle statistiche.\\\hline 
+ changeLanguage(in String) & Modifica l'URL del browser per cambiare lingua,
utilizzando la gestione automatica dell'internazionalizzazione offerta da
GWT.\\\hline
...
informazioni complete riguardanti la canzone data in input. In questo
...
nel \co{MusicLibrary} persistente le statistiche relative al catalogo
...
l'unicit\`a e ritorna 1 se ha successo, viene utilizzata all'interno di
...
Le classi di persistenza presenti in questo package, inoltre, implementano le
interfacce \co{Serializable} e \co{Cacheable} poich\'e sono gestite anche come
entit\`a della App Engine Memcache. 
\subsubsection*{Business logic interna} Nonostante il design pattern DAO non lo
preveda vengono implementate alcune componenti di business logic all'interno
delle classi di persistenza. Questo va leggermente a scapito della riusabilit\`a
ma viene giustificato dal fatto che la logica di business qui trattata \`e
molto semplice e strettamente legata all'implementazione interna dei DAO. Una
...
dunque questa natura ibrida tra classe di persistenza e business object.
...
riguardanti l'utente ed il suo catalogo. Estende le interfacce \co{Serializable}
...
\end{figure}
Il metodo \co{load()} che viene invocato tramite chiamate RPC di tipo
...
ricavarne l'oggetto persistente. Per gestire nel modo pi\`u appropriato il
...
persistent, anche se con un vantaggio meno evidente.
\subsubsection*{Attivit\`a svolte e dati trattati} Gestisce la
logica di persistenza di tutti i dati relativi ad un utente del sistema NetMus. Gestisce inoltre alcuni elementi di business logic strettamente legati a questa entit\`a come la ricerca di particolari utenti all'interno del Datastore.
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Attributo} & \bo{Descrizione} \\
\hline
...
dall'utente alla canzone. Questo garantisce un'ottima efficienza grazie alla
...
classe di business logic piuttosto che una classe di persistenza.
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Attributo} & \bo{Descrizione} \\
\hline
...
\endlastfoot
 ...
 - song\_list: Map\textless String, String\textgreater & Mappa che 
...
oggetti possono attingere. Fornisce inoltre la business logic necessaria alla
...
interfaccia che contiene la definizione dei metodi necessari e sufficienti per
...
completamente trasparente rispetto all'implementazione tramite JCache e
...
\endfoot
\endlastfoot
 - user\_name: String & indirizzo e-mail valido e univoco dell'utente.\\\hline
...
\end{itemize}
\subsubsection*{Tipo, obiettivo e funzione del componente}
Tutti le convenzioni riguardanti le stringhe con funzioni ben precise utilizzate
su netmus vengono qui definite e sono quindi visibili all'intero sistema.
Impostantissime sono i metodi di generazione degli id in cui sono
racchiuse le regole di costruzione delle chiavi che identificano le varie
entit\`a di Netmus. Questa classe contiene inoltre i metodi statici necessari a
validare con livello di sicurezza accettabile i dati inseriti dall'utente, si
rende fondamentale nelle operazioni di autenticazione e registrazione. 
\subsubsection*{Attivit\`a svolte e dati trattati} Qui vengono gestite tutte le
stringhe ricevute dai file caricati dagli utenti o gi\`a presenti in memoria su
cui c'\`e bisogno di fare dei controlli o dei matching.
Per quanto riguarda le
funzionalit\`a legate al login, invece, Le stringhe che vengono trattate in
questa classe sono rappresentate da password, username e indirizzi email. Per le
prime due tipologie di informazione il controllo eseguito si limita alla
lunghezza della stringa, per quanto riguarda gli indirizzi email, invece, viene
verificata l'aderenza agli standard.
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Attributo} & \bo{Descrizione} \\
\hline
...
\endfoot
\endlastfoot
 - SEPARATOR: String \emph{static final default= "-vtg-"} & Separatore
 final che viene utilizzato nella generazione degli id.\\\hline
\caption{Attributi di FieldVerifier}
\end{longtable}
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Metodo} & \bo{Descrizione} \\
...
\endfoot
\endlastfoot
 + cleanString(in String): String \emph{static} & Converte la string in input in
 una string pulita da ogni carattere diverso da lettere non accentate e numeri Ritrona
 stringa vuota in caso di valore null.\\\hline
 + generateAlbumId(in String,in String): String \emph{static} & Genera
 l'id corrispondente ai dati dell'album dati in input.\\\hline
 + generateSongId(in String,in String, in String): String \emph{static} & Genera
 l'id corrispondente ai dati della canzone dati in input.\\\hline
 + generatePlaylistId(in String,in String): String \emph{static} & Genera
 l'id corrispondente ai dati della playlist dati in input.\\\hline
...
\caption{Metodi di FieldVerifier}
\end{longtable}
...
\subsection*{Attivitca svolte e dati trattati}
...
in buona parte gi\`a codificato non \`e tracciabile poich\'e ne manca la
codifica della parte grafica e di conseguenza non \`e disponibile tra le funzionalit\`a
attive. Sar\`a sicuramente disponibile agli inizi delle fare di manutenzione ed
aggiornamento.\\
Il requisito C1FO-1.7.2 richiedono uno studio aggiuntivo
...
per\`o colmata nell'ultima versione di Firefox 4.0 RC.
