...
\hline
25/02/2011 & 0.23 & Palazzin Alberto & Agginta la descrizione e le tabelle
del package client.mvp.\\
\hline
24/02/2011 & 0.22 & Baron Federico & Agginta la descrizione e le tabelle
delle classi ProfilePlace e FieldVerifier.\\
\hline
24/02/2011 & 0.21 & Baron Federico & Agginta la descrizione e le tabelle
delle classi SongDTO e SongSummaryDTO.\\
\hline
23/02/2011 & 0.20 & Caputo Cosimo & Agginti i diagrammi UML di sequenza
...
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
 ...
 \co{ProfileActivity}.
 \subsubsection*{Attivit\`a svolte e dati trattati}
Contiene il catalogo dei brani dell'utente attraverso il quale è possibile
visualizzare tutte le informazioni rielative ad un brano e ordinare gli stessi
...
account.
\subsubsection*{Classi Interne} Viene implementata la classe interna \co{Song} la quale estende la classe astratta \co{Song} derivata da, e definita in, ProfileView. Lo scopo di questa classe è quello di rappresentare un oggetto che contenga al suo interno \emph{titolo}, \emph{autore} e \emph{album} di ciascun brano del catalogo. Questo risulta molto utile anche nella gestione di molte Widget GWT predisposte per elaborare in input liste di oggetti \emph{List\textless Type\textgreater}. Viene inoltre ridefinito il metodo \emph{equals(Song)} il quale consente di confrontare l'uguaglianza di due brani in base ai loro attributi.
...
\emph{MyCellTableResourcesPlaylist }, le quali estendono intrambe
...
@UiField numero\_brani: Label & Eticehtta che rappresenta il numero di
...
titolo selezionato nella playlist e che può essere rimosso.\\\hline
...
@UiField edit\_profile\_name: Label & Eticehtta  nome
dell'utente nella finestra dell'account.\\\hline
@UiField edit\_profile\_surname: Label & Eticehtta cognome
...
dove viene inserito evisualizzato il catalogo delle canzoni.\\\hline
...
left\_panel che contine l'elenco delle playlist dell'utente.\\\hline
@UiField friends: HTMLPanel & Elemento inserito all'interno di
left\_panel che contiene l'elenco degli amici su netmus.\\\hline
@UiField firends\_titolo: HTMLPanel & Elemento che contiene il titolo
...
a scomparsa che contiene le informazioni sull'accont.\\\hline
...
@UiField play\_youtube: Image & Rappresenta il bottne play nell'area del
...
funzionalità social di netmus\\\hline
...
@UiField edit\_profile\_checkImg: Image & Rappresente l'immagine di
...
dell'account.\\\hline
@UiField edit\_profile\_vc: VerticalPanle & Rappresenta una componente di layout
...
- playn: intn & Rappresenta se è in riproduzione un brano contenuto in una
...
UiBinder aggiungono funzionalità specifiche ai singoli elementi
semplicemente utilizzanod l'annotazione ``@UiHandler'' per riferirsi alle varie
...
\subsubsection*{Dettaglio del caricamento profilo}
...
\begin{itemize}
  \item Creazione ed esecuzione di un nuovo Place tramite il metodo \co{goTo()};
...
\co{NetmusActivityMapper}, si occuperanno di inizializzare opportunamente la
...
\co{ClientFactory}, rende possibili il riferirimento e la modifica delle
View.\\\hline
...
al \co{PlaceController}, all'\co{EventBus} e alle verie View.\\\hline
...
\co{ProfileView} per andare a graficare la lista delle playlist
...
\co{ProfileView} per andare a graficare la lista di utenti affini generata da
...
\co{ProfileView} per andare a graficare le informazioni del brano in
...
\`e creata da \co{ProfileView} utilizzata da \co{ProfileActivity}. 
\subsubsection*{Attivit\`a svolte e dati trattati}
...
utilizzate le classi \co{MusicLibrary SummaryDTO}, \co{MusicLibraryDTO},
...
sia presente almeno il titolo altrimenti il brano non verrÃ  mantenuto nel
...
nuovi brani insertiti nel DataStore, per associare ad essi il codice utile alla
riproduzione video.
\subsection*{Relazioni d'uso di altre componenti}
Vengono utilizzate le librerie \emph{gdata} di Google, create appositamente per
utilizzare pi\`u ad alto livello le sue API.
\subsection*{Interfacce con e relazioni d'uso da altre componenti}
Nella classe \co{Utils} verr\`a utilizzato il metodo di ricerca offerto da
\co{YouTubeManager}.
\subsection*{Attivit\`a svolte e dati trattati}
...
Nessuna.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Viene utilizzata in \co{YouTubeVideo}.
\subsubsection*{Attivit\`a svolte e dati trattati}
...
\subsubsection*{Relazioni d'uso di altre componenti}
Utilizza la classe \co{YouTubeMedia}.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Viene utilizzato per gestire i risultati di una ricerca in \co{YouTubeManager}.
\subsubsection*{Attivit\`a svolte e dati trattati}
...
\subsubsection*{Relazioni d'uso di altre componenti}
Utilizza la classe \co{YouTubeVideo} e le librerie \emph{gdata} di Google.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Viene utilizzata da \co{Utils} per effettuare una ricerca video su YouTube.
\subsubsection*{Attivit\`a svolte e dati trattati}
Gestisce una richiesta di ricerca video e deve principalmente restituire con
il codice del video pi\`u attinente alle keywords in ingresso nel metodo
\emph{getSearchResult(String keywords)}.
...
String contenente le keywords ed il tempo di timeout.\\\hline
...
\subsubsection*{Relazioni d'uso di altre componenti} Nessuna.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
\co{LoginActivity} e \co{ProfileActivity} ne fanno uso ogni qualvolta ricevano
...
limita alla lunghezza della stringa, per quanto riguarda gli indirizzi emial,
...
\endlastfoot
 ...
 in input si linga almeno 4 caratteri.\\\hline 
