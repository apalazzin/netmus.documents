...
\section{Codici assegnati alle nuove componenti}
...
EditUserPlace.Tokenizer.class, EditSongsPlace.Tokenizer.class\}) &
Questa annotazione che precede la definizione dell'interfaccia permette
di associare ogni place al \co{PlaceHistoryHandler} e quindi risparmia
...
\begin{itemize}
    \item C2QD-4.3 Supporto multi-lingua
    \item C2FD-2 Aggiornamento e completamento informazioni
...
\endfoot
\endlastfoot
- ROOT\_NAME: String \emph{static final default = ``Library''} & Nome del nodo
principale nel documento XML.\\\hline
- SONG\_NAME: String \emph{static final default = ``Song''} & Nome del nodo nel
documento XML riguardante una singola canzone.\\\hline
- ALBUMTITLE\_NAME: String \emph{static final default = ``AlbumTitle''} & Nome
del nodo nel documento XML riguardante il titolo dell'album.\\\hline
- AUTHORCOMPOSER\_NAME: String \emph{static final default = ``AuthorComposer''}
& Nome del nodo nel documento XML riguardante il compositore.\\\hline
- LEADARTIST\_NAME: String \emph{static final default = ``LeadArtist''} & Nome
del nodo nel documento XML riguardante l'artista.\\\hline
- SONGGENRE\_NAME: String \emph{static final default = ``SongGenre''} & Nome del
nodo nel documento XML riguardante il genere musicale del brano.\\\hline
- SONGTITLE\_NAME: String \emph{static final default = ``SongTitle''} & Nome del
nodo nel documento XML riguardante il titolo del brano.\\\hline
- TRACKNUMBER\_NAME: String \emph{static final default = ``TrackNumber''} & Nome
del nodo nel documento XML riguardante il numero della traccia all'interno dell'album.\\\hline
- YEAR\_NAME: String \emph{static final default = ``Year''} & Nome del nodo nel
documento XML riguardante l'anno di uscita del brano.\\\hline
- FILE\_NAME: String \emph{static final default = ``File''} & Nome del nodo nel
documento XML riguardante il file MP3.\\\hline
- document: Document & The DOM document element.\\\hline
- root: Element & The root node of the document.\\\hline
...
\endfoot
\endlastfoot
% non utilizzate, non � il caso di metterle nella documentazione
%+ reset() & \\\hline
%+ DTOtoXML(in SongDTO) & \\\hline
%+ generateXML(): String & \\\hline
%-------------------------------------------
- generateDTO(in Element): SongDTO & Da un nodo Song crea un nuovo \co{SongDTO}
che ne rispecchia le informazioni.\\\hline 
+ XMLToDTO(in String): List\textless SongDTO\textgreater & Da una
stringa contenente un documento XML, genera una lista di \co{songDTO}
utilizzando il metodo \co{GenerateDTO}.\\\hline
...
Nessuna.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Viene usata da \co{AppletBar} per visualizzare in maniera corretta lo stato
...
\begin{itemize}
    ...
    \item C2QD-4.2 Semplicit\`a di utilizzo
    \item C2FD-1.4 Informazioni dall'hard disk
\end{itemize}
\subsubsection*{Tipo, obiettivo e funzione del componente}
...
\subsubsection*{Relazioni d'uso di altre componenti}
Utilizza \co{AppletConstrants} per la traduzione dell'interfaccia con l'utente,
\co{TranslateDTOXML} per la comunicazione con l'applet, e \emph{server} per
...
\endfoot
\endlastfoot
- APPLET\_BAR: AppletBar \emph{static} & Istanza attuale di AppletBar.\\\hline
- visible: boolean & Indica se \`e visibile all'utente o no.\\\hline
- constants: AppletConstants & Classe utilizzata per la traduzione nella
lingua corretta.\\\hline 
- title: Label & Elemento grafico contenente il nome della barra da
visualizzare.\\\hline 
- on\_off: Anchor & Link per attivare/disattivare l'applet.\\\hline
- rescan: Anchor & Link per comunicare all'applet di riscansionare
l'ultimo device.\\\hline 
- status: TextBox & Elemento grafico che visualizza lo stato attuale.\\\hline
- applet: HTML & Elemento HTML che contiene l'applet.\\\hline
- user: String & Nome utente.\\\hline
- original\_user: String & Utente completo (email).\\\hline
- state: boolean & Indica se la scansione automatica \`e attivata o no.\\\hline
- translator: TranslateDTOXML & Oggetto utilizzato per la traduzione da
XML a DTO\\\hline
- library\_service: LibraryServiceAsync & Riferimento al service del
server.\\\hline
...
\endfoot
\endlastfoot
+ get(in String, in boolean): AppletBar \emph{static} & Nel caso non
esista, crea un'istanza di AppletBar. Imposta il nome utente corretto, e
restituisce l'istanza dell'oggetto.\\\hline
+ appletBarON() & Se non \`e gi\`a stato fatto, carica l'applet nel codice
HTML.\\\hline
+ appletBarOFF() & Rimuove l'applet dal codice HTML.\\\hline
- changeState() & Attiva/disattiva la scansione automatica.\\\hline 
- reScanAll() & Indica all'applet di riscansionare l'ultimo device.\\\hline
- sendStarts() & Inizializza l'applet (richiamando il metodo
\co{sendStartsJSNI}.\\\hline 
- sendStartsJSNI(in String, in boolean) \emph{native} & Invia all'applet
i parametri per l'inizializzazione.\\\hline
- scanningStatus(in int, in int) & Mostra graficamente lo stato di avanzamento
della scansione.\\\hline 
+ showStatus(in String) & Mostra graficamente lo stato dell'applet.\\\hline
+ translateXML(in String) & Elabora il risultato della scansione.\\\hline
+ makeNativeFunction(in AppletBar) \emph{native} & Effettua il
collegamento dei metodi in JSNI in modo da renderli utilizzabili
dall'applet.\\\hline
...
\co{LoginHelper}. \\
La comunicazione con il Datastore avviene tramite \co{UserAccount}.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Viene usata dal \emph{client} che ne genera un istanza tramite deferred binding,
...
utilizza la classe con visibilit\`a \emph{package} \co{ODF}. \subsection*{Relazioni d'uso di altre componenti} Vengono utilizzate le classi
DTO presenti nel package \emph{shared} per impacchettare i dati in uscita dal
database o per gestire i dati in entrata. 
\subsection*{Interfacce con e
...
\co{LoginDTO}, \co{UserSummaryDTO}, \co{UserDTO} e \co{UserCompleteDTO} come
...
\co{SongSummaryDTO} e \co{SongDTO} come input ed output in alcuni metodi.\\ Per interfacciarsi al database utilizza \co{ODF}.
...
Nessuna.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Verr\`a concretizzata da \co{LoginGoogleServlet}.s
\subsubsection*{Attivit\`a svolte e dati trattati}
...
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
Utilizzata da \co{DeviceScanner}.
...
\chapter{REQUISITI DELLA VERGOGNA}
...
C2QD-1.7 Ottimizzazione memoria cache\\
C2FO-1.3 Informazioni senza connessione\\
C1FD-1.10 Invio nuove informazioni a C2 \\
C2FN-1.5 File ignorati\ldots da spostare su server. \\ 
