...
\end{itemize}
\subsubsection*{Tipo, obiettivo e funzione del componente}
Questa classe consente alle componenti Activity di \emph{client} di inviare
richieste di vario tipo a \emph{server} riguardanti la fase di autenticazione e
logout degli utenti. \`E presente anche \co{LoginServiceAsync}, che gestisce
...
\rowcolor{orange} \bo{Metodo} & \bo{Descrizione} \\
\hline
+ insertRegistration(LoginDTO): boolean & Inserisce i dati di login
relativi alla registrazione di un nuovo utente nel database. Pu\`o
lanciare eccezioni di tipo \co{LoginException}.\\\hline 
+ verifyLogin(LoginDTO): boolean & Metodo fondamentale per l'utenticazione
che confronta i dati in input con quelli presenti nel database per verificare se
il login \`e valido.\\\hline
+ startLogin(LoginDTO) & Inizia la sessione dell'utente che si \`e autenticato
aggiornando anche i suoi attributi in \co{UserAccount}.\\\hline  
+ getLoggedInUserDTO(): UserSummaryDTO & Esegue una ricerca della sessione
corrente utilizzando i metodi di \co{LoginHelper} e ritorna, se presenti, le
informazioni di base (\co{UserSummaryDTO}) dell'utente loggato\\\hline 
+ logout() & Invalida l'autenticazione dell'utente che viene rimandato alla
pagina di login.\\\hline
\end{longtable}
...
\rowcolor{orange} \bo{Attributo} & \bo{Descrizione} \\
\hline
- NUM\_RETRIES: int = 5 & Numero di tentativi di accesso al database con
JDO e con controllo delle transazioni.\\\hline
\end{longtable}
\begin{longtable}{|p{0.4\textwidth}|p{0.4\textwidth}|}
\hline
\rowcolor{orange} \bo{Metodo} & \bo{Descrizione} \\
...
PersistenceManager pm): UserAccount & Legge dal database le informazioni
