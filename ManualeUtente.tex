...
\hline
\endfoot
10/03/2011 & 1.1 & Daminato Simone & Inseriti i messaggi di errore.\\
\hline
...
Vediamo qui di seguito come utilizzare NetMus.
...
di essa: può comunque essere bloccata cliccando sul lucchetto presente
...
e pausa dello stream audio, passaggio al brano precedente e successivo del catalogo, mentre il player
...
l'apposito pulsante sula player in alto (1), oppure chiudere totalmente la
...
``+'' nel men\`u a sinistra di fianco alla voce ``PLAYLIST''(1). Subito sotto comparir\`a un campo in cui
...
finestrella con l'elenco delle informazioni riguardanti il brano (1).\\
...
auto-generer\`a un PDF con l'elenco dei brani del catalogo, e visualizzer\`a il
documento generato, dove l'utente potr\`a guardarlo, stamparlo e/o salvarlo.
...
\begin{description}
	...
	\item[Errore: PDF non generato :] non \`e stato possibile generare il PDF,
	molto probabilmente dovuto ai servizi esterni su cui si appoggia questa
	funzione: riprovate in un secondo momento.
