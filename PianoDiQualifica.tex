...
\hline
\endfoot
27/02/2011 & 2.6 & Lovato Daniele & Inseriti esiti test di unit\`a.\\
\hline
20/02/2011 & 2.5 & Lovato Daniele & Inseriti altri test di unit\`a.\\
\hline
17/02/2011 & 2.4 & Palazzin Alberto & Inserita la sezione riguardanti i
metodi per la verifica della ``Progettazione nel dettaglio'' e
della ``Codifica''.\\
\hline
11/02/2011 & 2.3 & Lovato Daniele & Inseriti alcuni test di unit\`a.\\
\hline
...
Utilizzare lo strumento Metrics per misurare:
\begin{itemize}
...
Tecniche: quelle usate da Metrics.
...
\textbf{Aspettative}: Il metodo dovr\`a ritornare una lista di DTO di tutti i
brani rilevati senza saltarne alcuno e con tutte le informazioni memorizzate nel
rispettivo campo.\\
...
l'estrazione e la visualizzazione non devono dare problemi\\
...
\textbf{Aspettative}: il getter deve restituire una lista di stringhe con i
nomi degli amici e il setter deve inviarla correttamente alla view per la
visualizzazione\\
...
\textbf{Aspettative}: il metodo deve permettere di registrare il voto di una
canzone memorizzandolo sul DB della relativa canzone e di passarlo alla view per
visualizzarlo\\
...
player di youtube\\
\textbf{Output Attesi}: la ricerca deve ottenere il brano corretto da youtube in
base alla canzone selezionata e deve aggiornare la view con la visualizzazione di
...
\textbf{Aspettative}: deve restituire una lista di stringhe contenenti
autore,titolo ed album corretti di ogni canzone della libreria dell'utente, e
deve permettere il corretto invio per la visualizzazione di tali informazioni
sulla view\\


\textbf{metodo}: removeFromPlaylist\\
\textbf{Obbiettivo del Test}: verificare la rimozione di un brano da una playlist\\
\textbf{Aspettative}: il metodo richiama la rimozione del brano dalla playlist
dell'utente sia dalla parte del DB che dalla parte della view\\

\item TU-Cclap1: LoginActivity\\
...
\textbf{Aspettative}: il metodo deve creare un loginDTO con le informazioni
inserite dall'utente, inviarlo al server per verificare la validit\`a e in caso
di successo aprire la pagina di profileView, in caso di insuccesso lanciare un
messaggio di errore a seconda della causa (login incorretto o errore di
sistema)\\

\item TU-Cse5:  UserServiceImpl\\
\textbf{metodo}: loadProfile\\
\textbf{Obbiettivo del Test}: verificare la corretta selezione dell'utente da
caricare e la restituzione dell'esatto UserCompleteDTO con tutti i campi
corretti\\
\textbf{Aspettative}: la restituzione di un UserCompleteDTO corretto
...
\textbf{Aspettative}: corretta memorizzazione all'interno del db dell'utente di
tutti i nuovi brani rilevati\\
...
\textbf{Aspettative}: il brano viene correttamente votato e registrato nel DB
inoltre che venga passato alla view il voto, e che il metodo restituisca la
media dei voti per quel brano\\
...
\textbf{Aspettative}: il metodo deve recuperare e confrontare se la sessione di
lavoro \`e attiva per un utente, in caso non lo fosse verificare se i cookie
sono settati per il recupero, in caso positivo recupera la sessione mentre in
caso negativo lancia una loginException\\

\item TU-Csepe5: UserAccount\\
\textbf{metodo}: findSessionUser\\
\textbf{Obbiettivo del Test}: verificare il funzionamento delle sezioni attraverso il
ritorno dell'user account corretto a partire dall'ID della sessione\\
\textbf{Aspettative}: l'utente viene caricato correttamente se loggato e la
sua sessione ripristinata restituendo un oggetto UserAccount\\
...
\textbf{Aspettative}: viene ritornato true qualora la cancellazione sia andata
a buon fine, e che nella libreria dell'utente il brano sia stato
effettivamente rimosso\\

\textbf{metodo}: addSong\\
\textbf{Obbiettivo del Test}: verificare la corretta registrazione di una nuova
canzone sul DB\\
\textbf{Aspettative}: il metodo deve ricercare nel DB la presenza della canzone
che si sta cercando di aggiungere. In caso di match positivo, la canzone non
viene aggiunta perch\`e gi\`a presente, in caso negativo viene registrata e i
...
\emph{AnalisiDeiRequisiti-2.0.pdf}. In dettaglio per motivi di
...
hanno lo scopo di soddisfare le richieste di 72 requisiti totali, 41 funzionali,
21 di qualit\`a e 10 di vincolo. Il
rapporto tra componenti e requisiti funzionali \`e considerato soddisfacente.
La distribuzione dei requisiti su tutti i moduli \`e equilibrata.
...
\begin{itemize}
  \item TU-Cclap3: TranslateDTOXML\\
  \textbf{metodo}: XMLToDTO\\
  \textbf{esito del test}: La lista di SongDTO \`e stata restituita
  correttamente compilata in tutti i suoi campi per qualunque brano\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Cclac2: ProfileActivity\\
  \textbf{metodo}: setPlaylistList/getPlaylistList\\
  \textbf{esito del test}: i due metodi comunicano correttamente, la view riceve
  i dati da visualizzare\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: setFriendList/getFriendList\\
  \textbf{esito del test}: i due metodi funzionano correttamente, la view riceve
  la lista con le stringhe di amici da memorizzare\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: rateSong\\
  \textbf{esito del test}: il voto viene passato alla view correttamente come
  pure viene passato al DB per la registrazione \\ 
  \textbf{malfunzionamenti rilevati}:nessuno\\
  
  \textbf{metodo}: playYouTube\\
  \textbf{esito del test}: il metodo funziona correttamente, viene richiesto
  il codice di youtube della canzone e correttamente passato alla view\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: setSongs/getSongs\\
  \textbf{esito del test}: la comunicazione tra i due metodi funziona
  correttamente, i brani vengono richiesti al DB e passati alla view\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: removeFromPlaylist\\
  \textbf{esito del test}: il metodo comunica correttamente con la view,
  richiedendo la rimozione del brano indicato\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Cclac1: LoginActivity\\
  \textbf{metodo}: sendLogin\\
  \textbf{esito del test}: la chiamata al DB viene effettuata correttamente, e
  i diversi casi di esito positivo o negativo del login vengono gestiti senza
  errori\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  ...
  \item TU-Cse5: UserServiceImpl\\
  \textbf{metodo}: loadProfile\\
  \textbf{esito del test}: viene richiesto e
  restituito UserCompleteDTO corretto\\ 
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Cse3: SongServiceImpl\\
  \textbf{metodo}: rateSong\\
  \textbf{esito del test}: la valutazione del brano ricevuta in input viene
  correttamente inviata al DB per essere memorizzata, e viene ritornata la
  media dei voti\\ 
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Cse4: LoginServiceImpl\\
  \textbf{metodo}: restartSession\\
  \textbf{esito del test}: le sessioni, qualora fossero valide per l'utente
  passato al metodo, vengono ripristinate correttamente e le informazioni
  recuperate\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Cse6: LibraryServiceImpl\\
  \textbf{metodo}: sendUserNewMusic\\
  \textbf{esito del test}: i brani ricevuti in input vengono correttamente
  inviati al DB dell'utente corretto\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Csepe5: UserAccount\\
  \textbf{metodo}: findSessionUser\\
  \textbf{esito del test}: viene caricato e restituito l'utente corretto
  tramite tipo di ritorno userAccount\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: deleteUser\\
  \textbf{esito del test}: l'utente da cancellare passato come stringa viene
  cancellato correttamente dal DB\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \item TU-Csepe2: MusicLibrary\\
  \textbf{metodo}: removeSong\\
  \textbf{esito del test}: viene rimosso correttamente il riferimento ad una
  canzone dalla libreria musicale dell'utente. La canzone resta tuttavia
  memorizzata nel DB centrale\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
  
  \textbf{metodo}: addSong\\
  \textbf{esito del test}: il metodo registra correttamente canzone e
  informazioni relative nel DB e conseguentemente nella libreria musicale
  dell'utente\\
  \textbf{malfunzionamenti rilevati}: nessuno\\
