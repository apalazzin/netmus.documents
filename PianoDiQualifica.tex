...
metodo: XMLToDTO\\
Obbiettivo del Test: questo metodo riceve in input una stringa XML che
...
Dati in input: verrà creata una stringa xml con una notevole quantità di brani
e rispettive informazioni. \\
Output Attesi: Il metodo dovrà ritornare una lista di DTO di tutti i brani\\
rilevati senza saltarne alcuno e con tutte le infomazioni memorizzate nel
rispettivo campo.
Output Ottenuti: \\
Maulfunzionamento Rilevati:\\
Costi e Rischi:\\

\item TU-Cclac2:  ProfileActivity\\
metodo: setPlaylistList/getPlaylistList\\
Obbiettivo del Test: Verificare che avvenga con successo l'estrazione dal DB
delle playlist e che vengano correttamente inserite nella view anche con una
notevole quantità di brani.\\
Dati in input: viene creato una lista fittizia di playlist.\\
Output attesi:  lista di String di playlist effettivamente memorizzate nel DB
dell'utente. \\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: setFirendList/getFriendList\\
Obbiettivo del test: verificare la lettura dal DB della lista di amici di un
utente, e verificare la corretta importazione e visualizzazione sulla view\\ 
Dati in input: lista fittizia in nomi degli amici dell'utente\\
Output attesi: la lista di Stringhe degli amici per quando riguarda il getter\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: setSongInfo/getSongInfo\\
Obbiettivo del test: verificare la corretta estrazione delle info di un brano
dal DB e la visualizzazione sulla view\\ 
Dati in input: un SongDTO con I dati estratti dal DB\\
Output attesi: Stringa con le info di un brano\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: setPlaylistSongs/getPlaylistSongs\\
Obbiettivo del test: verificare la corretta estrazione e visualizzazione dei
dati dal DB alla view dei brani di una playlist\\ 
Dati in input: una stringa contentente il titolo di una playlist\\
Output attesi: una lista di stringhe contententi le informazioni corrette
riguardanti I brani della corretta playlist per il getter, una corretta
visualizzazione per il setter\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: playYouTube/getYouTubeLink\\
Obbiettivo del test: verificare che il programma sia in grado di ottenere e
successivamente di impostare e visualizzare una canzone in streaming tramite
player di youtube\\
Dati in input: Stringhe con autore, titolo e album del brano che si vuole
ascoltare\\ 
Output attesi: la ricerca deve ottenere il brano corretto da youtube in base
...
metodo: setSongs/getSongs\\
Obbiettivo del test: verificare la corretta estrazione e visualizzazione del
catalogo brani, qualora un utente sia loggato\\ 
Dati in input: un MusicLibrarySummaryDTO per quanto riguarda il getter\\
Output attesi: Una lista di stringhe contenenti autore,titolo ed album\\
...
metodo: addToPlayList\\
Obbiettivo del test: si vuole verificare che venga inserito correttamente un
nuovo brano ad una playlist nella view.\\ 
Dati in input: Stringhe di informazioni del brano da inserire\\
Output attesi: Una view ben formattata con all'interno della view una playlist
col brano giusto inserito \\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: removeFromPlaylist\\
Obbiettivo del test: verificare la rimozione di un brano da una playlist\\
Dati in input: stringhe di informazioni del brano da inserire\\
Output attesi: Una view ben formattata con la rimozione del brano effettuata\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: addPlaylist\\
Obbiettivo del test: verificare che una volta che l'inserimento nel DB della
playlist ha successo, venga correttamente inserita e visualizzata nella view\\ 
Dati in input:titolo della playlist\\
Output attesi: corretta visualizzazione della playlist\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: removePlaylist\\
Obbiettivo del test:\\
Dati in input:\\
Output attesi:\\
Output Ottenuti:\\
...
metodo:  loadLibrary\\
Obbiettivo del test: verificare l'estrazione della libreria dei brani
dell'utente dal DB \\
Dati in input: Username e password dell'utente\\
Output attesi: Un MusicLibraryDTO che contiene le informazione esatte della
...
metodo: addSong\\
Obbiettivo del test: verificare la registrazione di un nuovo brano nella
libreria dell'utente all'interno del database\\ 
Dati in input: un SongDTO con le informazione complete o incomplete
riguardanti il brano \\
Output attesi: memorizzazione sul database del brano corretto\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\


metodo: sendUserNewMusic\\
Obbiettivo del test: Verificare l'invio e la memorizzazione di una lista di
brani nuovi rilevati dal dispositivo dell'utente\\ 
Dati in input: Stringa nome utente e lista di SongDTO\\
Output attesi: corretta memorizzazione all'interno del db dell'utente di tutti
...
metodo: getLibrary\\
Obbiettivo del test: verificare la corretta creazioe del DTO che contiene le
informazioni riguardanti la libreria musicale dell'utente\\ 
Dati in input: Stringa contenente il nome dell'utente\\
Output attesi: un MusicLibrarySummaryDTO contenente le informazioni corrette
...
metodo: findSessionUser
Obbiettivo del test: verificare che il funzionamento delle sezioni attraverso il
ritorno dell'user account corretto a partire dall'ID della sessione\\
Dati in input: Stringa con ID di sessione\\
Output attesi: un oggetto di tipo userAccount dell'utente esatto\\
Output Ottenuti:\\
Malfunzionamento Rilevati:\\
Costi e Rischi:\\

metodo: deleteUser\\
Obbiettivo del test: verificare l'effettiva e corretta cancellazione di un
account dal DB e quindi dal sistema NetMus\\
Dati in input: Stringa col nome dell'utente\\
Output attesi: tipo di ritorno void, ma ciò che si aspetta è che l'account
...
metodo: removeSong\\
Obbiettivo del test: verificare la corretta rimozione del riferimento ad una
canzone dalla libreria nel DB di un utente\\
Dati in input: un oggetto Song che rappresenta la canzone da rimuovere e un
booleano update\\
Output attesi: un booleano con true o false a seconda se la rimozione ha avuto
successo oppure no\\
Output ottenuti: \\
Malfunzionamenti rilevati:\\
Costi e Rischi:\\
...
\bo{Ambiguità} & \bo{Verificabilità}  & \bo{Consistenza}  & \bo{Tracciabilità}
\\
\hline
\endhead
\endfoot
...
tracciata mentre Ccl3 è stata tracciata per le seguenti motivazioni:
\begin{itemize}
