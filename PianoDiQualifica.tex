...
\textbf{Strategia}: verra passato al metodo getter un DTO con le informazioni
riguardanti la libreria di un utente, il metodo si occupa semplicemente di
smistare e riorganizzare le canzoni in una lista di strighe che verr\`a
...
ogni canzone della libreria dell'utente, e deve permettere il corretto invio per la visualizzazione di tali informazioni sulla view\\
...
presenza di tale utente nel DB locale dovr\`a dare esito positivo di login. In
caso di errore verr\`a ritornato un messaggio di errore di utente non
registrato, oppure di errore di connessione al DB\\ 
\textbf{Aspettative}: il
metodo deve creare un loginDTO con le informazioni inserite dall'utente, 
inviarlo al DB locale per verificare la validit\`a e in caso di successo aprire la pagina di profileView\\
...
\begin{itemize}
  ...
  \begin{itemize}
  \item Lines of Code: \bo{5.898};
  \item Number of Types: \bo{162} (23 interfacce, 139 classi);
  \item Afferent Couplings: \bo{0};
  \item Efferent Couplings: \bo{77};
  \item Number of Methods: \bo{703};
  \item Number of Methods Per Type: media=\bo{4.33}, max=\bo{91};
  \item Number of Parameters: media=\bo{1.01}, max=\bo{8};
  \item Lines Of Code Per Method: media=\bo{7.41}, max=\bo{204};
  \item Block Depth: media=\bo{0.77}, max=\bo{5};
  \item Cyclomatic Complexity: media=\bo{1.55}, max=\bo{19}.
  \end{itemize}
    \item \bo{Applet:}
  \begin{itemize}
...
\\\\
Il livello di annidamento medio per il sistema generale \`e 0.77, mentre per
l'applet \`e 1.11. Questo significa che si \`e cercato di mantenere un
architettura non troppo complessa.
\\\\
Il numero medio di metodi per classe e interfaccia per il sistema generale \`e
4.33, mentre per l'applet \`e 4.00. Entrambi i risultati son buoni perch\'e
...
\begin{itemize}
     \item \bo{Sistema generale:}
  \begin{itemize}
  \item Number of Types: it.unipd.netmus.client.ui con valore=\bo{76};
  \item Number of Methods Per Type: MyConstants.java con valore=\bo{17.00};
  \item Number of Methods Per Type: Song.java con valore=\bo{40.00};
  \item Number of Methods Per Type: UserAccount.java con valore=\bo{32.00};
  \item Number of Methods Per Type: SongDTO.java con valore=\bo{22.00};
  \item Number of Methods Per Type: UserDTO.java con valore=\bo{20.00};
  \item Lines of Code: ProfileViewImpl.java con valore=\bo{1,707};
  \item Block Depth: LoginGoogleCallbackServlet.java con valore=\bo{5.00};
  \item Cyclomatic Complexity: Song.java con valore=\bo{1.80}; 
  \item Cyclomatic Complexity: BCrypt.java con valore=\bo{4.14}.
  \end{itemize}
    \item \bo{Applet}:
  \begin{itemize}
  \item Lines Of Code Per Method: TranslateXML.java con valore=\bo{47.33};
...
Tutti i test in cui \`e coinvolta la classe \co{TranslateDTOXML} evitano di
...
Considerando l'esito della revisione RP e le
