...
\newcommand{\datamodifica}{14 Gennaio 2011}
...
\vspace*{0.5cm}
\bo{Use case UC1A: Sistema NetMus (1/2)}\\\\
\bo{Attori primari: } Utente \\\\
\bo{Pre-condizioni: } L'utente \`e connesso ad internet ed accede al portale
NetMus tramite un browser. \\\\ 
\bo{Post-condizioni: } Il portale NetMus ha soddisfatto tutte le richieste
dell'utente. \\\\ 
...
\vspace*{0.5cm}
\bo{Use case UC1B: Sistema NetMus (2/2)}\\\\
\bo{Attori primari: } Utente \\\\
\bo{Pre-condizioni: } L'utente \`e connesso ad internet ed accede al portale
NetMus tramite un browser. \\\\ 
\bo{Post-condizioni: } Sono state inviate al server tutte le
...
\bo{Pre-condizioni: } L'utente \`e connesso ad internet, accede al portale
...
%  \item C1 trova alcuni brani gi\`a in memoria e scarta i doppioni in entrata.
%  Nel farlo, se le informazioni non combaciano precisamente, valuta quale delle
%  due versioni \`e la pi\`u completa e adotta una strategia di
%  ``\underline{merge}'' che minimizzi la ridondanza.
%  \item Per i brani che non sono gi\`a presenti nel database C1 effettua una
%  ricerca all'esterno e se trova informazioni aggiuntive per i brani in arrivo
%  le inserisce.
...
%  \begin {enumerate}
%    \item Queste informazioni vengono scartate senza possibilit\`a di
%    ripristino e l'elaborazione procede senza conseguenze.
%  \end{enumerate}
