...
proprone di soddisfare nello sviluppo del software richiesto dal capitolato
d'appalto 02 NetMus. Per favorire la comprensione dell'analisi proposta sono
descritti alcuni use-case che comprendono la quasi totalità dei requisiti
elaborati.
Nel presentare questa Analisi dei requisiti si è scelto di separare chiaramente
le due componenti fondamentali, quella di invio dati e quella di persistenza e
visualizzazione, che costituiscono il sistema NetMus poichè dovranno essere il
...
richieste del capitolato d'appalto 02 NetMus. Con il termine requisiti si
vogliono indicare sia le funzionalità attese nel prodotto finale sia gli
strumenti adottati per raggiungere tale risultato.
...
architetturale che reppresenta ad alto livello come sar\`a strutturato l'intero
...
\begin{center}
	...
	\emph{Figura 1: diagramma architetturale}
\end{center}
...
informazioni riuguardanti nuovi brani musicali presenti nel computer o altre
...
\begin{itemize}
  ...
  \begin {enumerate}
    \item Il tentarivo di autenticazione fallisce.
  \end{enumerate}
\end{itemize}
\newpage
...
\bo{Pre-condizioni: } L'utente ha aperto l'applicazione da browser ed è
autenticato. \\\\ 
...
\begin{itemize}
  ...
  \begin {enumerate}
    ...
    \item In caso affermativo l'utente può procede a visualizzare altri profili,
    atrimenti gli viene negato l'accesso e viene rimandato alla propria pagina.
  \end{enumerate}
\end{itemize}
\newpage
...
\bo{Pre-condizioni: } L'utente ha aperto l'applicazione da browser ed è
autenticato. \\\\ 
\bo{Post-condizioni: } L'utente è riuscito a vedere tutte le informazioni
...
\begin{enumerate}
  ...
  oppurtunamente raggruppate ed un menù sul lato sinistro per facilitare la
  navigazione all'interno della pagina, la visualizzazione è simile a quella
  del software iTunes.
  \item L'utente seleziona il brano che gli interessa e ne visualizza i
  dettagli tra cui c'è anche il player di YouTube che permette di ascoltare la
  canzone in streaming.
  \item In ogni momento, sia nella visualizzazione del catalogo sia in quella
  del singolo brano, l'utente può modificare varie opzioni e cancellare o
...
\begin{itemize}
  ...
  \begin {enumerate}
    \item Il player non viene visualizzato ma tutte le altre funzionalità
    rimangono invariate.
  \end{enumerate}
\end{itemize}
\newpage
...
\begin{enumerate}
  ...
  \item C1 controlla che non vi siano dati maligni o non riguardinti brani
  musicali.
  \item C1 effettua una ricerca nel proprio database per verificare se alcuni
  brani sono già presenti.
  \item C1 trova alcuni brani già in memoria e scarta i doppioni in entrata. Nel
  farlo, se le informazioni non combaciano precisamente, valuta quele delle due
  versioni è la più completa e adotta una strategia di ``merge'' che minimizzi
  la ridondanza.
  \item Per i brani che non sono già presenti nel database C1 effettua una
...
\begin{itemize}
  ...
  \begin {enumerate}
    \item Queste informazioni vengono scartate senza possibilità di ripristino e
  ...
  \begin {enumerate}
    \item Vengono tenuti entrambi i brani, fino a quando il sistema avrà
...
\begin{center}
	\includegraphics[width=16cm]{img/AR/GerarchiaRequisiti.png}
	
	\emph{Tabella 1: gerarchia dei requisiti}
\end{center}

\subsection{Gerarchia dei requisiti - Tipologia}
\begin{center}
	\includegraphics[angle=90,height=23cm,width=8cm]{img/AR/RequisitiFunzionali.png}
	
	\emph{Tabella 2: requisiti funzionali}
	
	\subsubsection{}
	
	\includegraphics[width=18cm]{img/AR/RequisitiDiQualita.png}
	
	\emph{Tabella 3: requisiti di qualit\`a}
	
	\subsubsection{}
	
	\includegraphics[width=12cm]{img/AR/RequisitiDiVincolo.png}
	
	\emph{Tabella 4: requisiti di vincolo}
\end{center}
\newpage
