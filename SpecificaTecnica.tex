...
\hline
\endfoot
18/03/2011 & 3.0 & Caputo Cosimo & Validazione per consegna RA.\\
\hline
18/03/2011 & 2.3 & Baron Federico & Verificato l'intero documento.\\
\hline
17/03/2011 & 2.2 &  Palazzin Alberto & HomePage aggiornata.\\
\hline
16/03/2011 & 2.1 & Palazzin Alberto & Apportate modifiche in base alla
correzione in sede di RQ.\\
\hline
27/02/2011 & 2.0 & Lovato Daniele & Validazione per consegna RQ.\\
\hline
26/02/2011 & 1.5 & Mandolo Andrea & Verificato l'intero documento.\\ 
\hline
25/02/2011 & 1.4 & Palazzin Alberto & Apportate modifiche in base alla
verifica preliminare.\\
\hline
19/02/2011 & 1.3 & Mandolo Andrea & Verifica preliminare del documento.\\
\hline
17/02/2011 & 1.2 & Caputo Cosimo & Apportate modifiche in base alla
correzione e alla progettazione di dettaglio.\\
\hline
...
Questo design pattern sar\`a utilizzato per definire
l'architettura della componente di persistenza e visualizzazione e
nel nostro caso sar\`a implementato con il framework MVP con Activities e Places
offerto da GWT 2.1 che semplifica la gestione di alcuni aspetti consolidati di
MVP. In questo framework le Activities svolgono il ruolo di Presenter e i Places
rappresentano gli stati della \underline{UI}.
...
\begin{figure}[h]
\centering
\includegraphics[width=15cm]{img/ST/DAO_DTO1.png}
\caption{Diagramma UML delle classi che descrive la struttura dell'utilizzo
simultaneo dei patterns DAO e DTO}
\end{figure}
Questi patterns vengono introdotti rispettivamente all'interno della componente
model (DAO) e presenter (DTO) di MVP e riguardano gli oggetti utilizzati per
l'incapsulamento dei dati nelle comunicazioni tra client, server e database. 
...
dell'applicazione, nel nostro caso le Activity di MVP. In Netmus definisce la
struttura delle classi \co{MusicLibrary}, \co{UserAccount} e \co{Song} contenute nel
package \emph{persistent}. L'implementazione \`e fondamentalmente una classe che
rappresenta un'entit\`a tabellare di un database. Questi oggetti estratti dal
database in alcuni casi possono essere utilizzati per la comunicazione con i
clients ma hanno pesanti restrizioni dovute alla grande quantit\`a di
informazioni e procedure che contengono, per questo motivo abbiamo deciso di
affiancare a DAO anche il pattern Data Transfer Object. Gli oggetti DTO,
contenuti nel package \emph{shared}, verranno scambiati nella comunicazione tra
client e server con il vantaggio di essere pi\`u essenziali dei DAO e si
essere costruiti su misura per la comunicazione (in seguito sar\`a illustrata
la struttura gerarchica che utilizza questo pattern). La logica del tracciamento
tra DAO e DTO sar\`a a carico degli oggetti DAO.
\\
...
\begin{figure}[h]
\centering
\includegraphics[width=16.5cm]{img/ST/AbstractFactory1.png}
...
sistema. L'interfaccia \co{ClientFactory} sar\`a utilizzata per ottenere le
interfacce degli elementi architetturali necessari alla nostra applicazione come
ad esempio l'event bus e le varie UI.
Per caricare le differenti implementazioni di \co{ClientFactory} a seconda delle
...
\begin{figure}[h]
\centering
\includegraphics[width=4cm]{img/ST/Singleton1.png}
...
e model attraverso le Activity, le interfacce Service e i DTO del package
\emph{shared}. La rappresentazione della dipendenza tra il server e il package
\emph{Service} \`e dovuta a meccanismi interni a GWT RPC che costituiscono la
base della comunicazione tra client e server.\\\\
...
Nel package \emph{client} troviamo la classe \co{Netmus} che rappresenta
l'entry point, occupandosi di generare tutte le componenti e di renderle
visibili. Nel sottopackage \emph{mvp} sono contenute le classi
\co{NetmusActivityMapper} e \co{NetmusPlaceHistoryMapper} funzionali al
meccanismo delle Activity e dei Place. Per soddisfare i requisiti funzionali del
sistema NetMus sono state individuate 4 componenti che implementano altrettante
UI con la relativa logica di attivazione delle Activities. Queste sono Login,
User, EditProfile e EditSongs che, grazie all'interazione tra le loro parti
view, place e activity, garantiscono all'utente tutte le funzionalit\`a
attese.\\ Le interfacce Service invece rappresentano il punto di collegamento
con il model la cui implementazione \`e sviluppata lato server. Queste
interfacce, infatti, vengono accedute dalle Activities poich\'e forniscono tutti
i metodi riguardanti la business logic.\\\\

\subsubsection{Decomposizione architetturale - lato server}

\begin{figure}[h]
  \centering
  \includegraphics[width=16.5cm]{img/ST/Server_Shared.png}
\caption{Diagramma delle classi dei packages server e shared}
\end{figure}

Nella parte server si trovano le implementazioni dei Service e i package
\emph{servlet}, \emph{persistent} e \emph{utils}. I Service hanno la
possibilit\`a di accedere alle informazioni contenute nel database grazie alle
classi DAO presenti nel package \emph{persistent}, le classi di \emph{servlet}
interfacciano il sistema con i servizi esterni (autenticazione Google, servizi
di tagging, Youtube) mentre in \emph{utils} sono contenute le classi di
utilit\`a generali del server. Grande importanza viene data al package
\emph{shared} poich\'e contiene il sottopackage \emph{exception} e la gerarchia
di classi che rende allo stesso tempo semplice ed efficiente il passaggio di
informazioni tra il server e i clients incapsulando gli oggetti di persistenza
DAO e rimuovendone la parte logica, nel rispetto del design pattern DTO. Lato
server compaiono inoltre le classi \co{PMF}, che si occupa di creare l'istanza
della classe \co{PersistanceManagerFactory} per la gestione della persistenza e
la comunicazione con il DataStore, e \co{loginHelper}, per le gestione delle
sessioni http.
...
\caption{Diagramma delle attivit\`a che descrive il procedimento di
...
durante l'attivit\`a di login, focalizzando sulla classe \co{LoginActivity}, che
svolge le centrali funzioni di richiedere e ricevere informazioni e servizi dal
server, e di creare e comunicare con i \co{ProfilePlace}. Di seguito gli step
fondamentali dell'operazione:
\begin{itemize}
  \item \co{LoginView} invia i dati della richiesta di accesso alla
  \co{LoginActivity};
  \item Tramite l'interfaccia \co{LoginService} i dati verranno inviati e
  controllati dal server e l'esito dell'autenticazione restituito;
  \item Una volta confermati i dati l'activity procede all'istanziazione del
  \co{ProfilePlace} che provveder\`a ad avviare la nuova activity tramite
  l'\co{ActivityMapper} e quindi a visualizzare la View del profilo autenticato.
\end{itemize}
...
da GWT 2.1). In seguito alla progettazione di dettaglio sono state incorporate
in questa view le funzionalit\`a di modifica delle informazioni personali
dell'utente e di modifica del catalogo brani, precedentemente svolte da
 \co{EditUserView} e \co{EditSongsView} (ora escluse dall'architettura).
...
In seguito alla progettazione di dettaglio si \`e scelto di non utilizzare
pi\`u questa classe. il suo ruolo verr\`a rivestito da \co{ProfileView}.\\
{\color{Removed_Grey}
\subsubsection*{Tipo, obiettivo e funzione del componente}
Il sistema mette a disposizione una vista per la modifica
delle informazioni personali dell'utente registrato. La classe \co{EditUserViewImpl} implementa
l'interfaccia \co{EditUserView} che estende \co{isWidget} (interfaccia
introdotta da GWT 2.1).
...
\co{EditSongsView} che estende \co{isWidget} (interfaccia introdotta da GWT 2.1).
\subsubsection*{Relazioni d'uso di altre componenti}
...
interno alla \co{LoginView} e richiede al \co{clientFactory} di implementarela.
...
In seguito alla progettazione di dettaglio si \`e scelto di non utilizzare
pi\`u questa classe. il suo ruolo verr\`a rivestito da \co{ProfileActivity}.\\
{\color{Removed_Grey}
\subsubsection*{Tipo, obiettivo e funzione del componente}
Questa classe estende \co{AbstractActivity}, implementa il
\co{Presenter} interno alla \co{EditUserView} e richiede al \co{clientFactory} di
implementarela.
\subsubsection*{Relazioni d'uso di altre componenti}
Invoca i servizi di interrogazione e modifica dei dati profilo forniti da
...
\subsubsection*{Tipo, obiettivo e funzione del componente}
Questa classe estende \co{AbstractActivity}, implementa il
\co{Presenter} interno alla \co{EditSongsView} e richiede al \co{clientFactory} di
implementarela.
...
In seguito alla progettazione di dettaglio si \`e scelto di non utilizzare
pi\`u questa classe. il suo ruolo verr\`a rivestito da \co{ProfilePlace}.\\
{\color{Removed_Grey}
\subsubsection*{Tipo, obiettivo e funzione del componente}
\co{EditUserPlace} estende la classe \co{Place} messa a
disposizione da GWT. La classe \`e associata alla classe interna \co{Tokenizer} che permette di
serializzare lo stato del Place in un simbolo (token) URL.
\subsubsection*{Relazioni d'uso di altre componenti}
Nessuna.
\subsubsection*{Interfacce con e relazioni d'uso da altre componenti}
...
{\color{Removed_Grey}
\subsubsection*{Tipo, obiettivo e funzione del componente}
\co{EditSongsPlace} estende la classe \co{Place} messa a
disposizione da GWT. La classe \`e associata alla classe interna \co{Tokenizer} che permette di
serializzare lo stato del Place in un simbolo (token) URL.
\subsubsection*{Relazioni d'uso di altre componenti}
Nessuna.
...
I seguenti requisiti sono risultati non tracciabili, in quanto non legati ad
una specifica componente strutturale, e appartengono alle seguenti
categorie:
\begin{itemize}
  \item vincoli;
...
La classe \co{Utils} \`e stata progettata per raccogliere tutte quelle funzioni
...
buona pratica includere \co{Utils} nella progettazione anche se non direttamente
