...
\co{LoginActivity} in modo da rendere eccessibili le funzionalit\`a di history e
...
\subsubsection*{Attivit\`a svolte e dati trattati}
Il compito della classe \`e quello di indicizzare gli stati di
\co{ProfileActivity} in modo da rendere eccessibili le funzionalit\`a di history
...
\subsubsection*{Attivit\`a svolte e dati trattati}
Il compito della classe \`e quello di indicizzare gli stati di
\co{EditUserActivity} in modo da rendere eccessibili le funzionalit\`a di history e
bookmarking del browser.
...
\subsubsection*{Attivit\`a svolte e dati trattati}
Il compito della classe \`e quello di indicizzare gli stati di
\co{EditSongsActivity} in modo da rendere eccessibili le funzionalit\`a di history e
bookmarking del browser.
...
presente un'interfaccia normale e un interfaccia \bo{asincrona}.
...
avranno relazioni d'uso definite, se non con gli oggetti DTO (\co{SongDTO},
\co{SongSummaryDTO}, \co{MusicLibraryDTO}) che verranno utilizzati per il
...
\subsection{Interfaccia UserService (e UserServiceAsync)}
\subsubsection*{Tipo, obiettivo e funzione del componente}
...
\co{UserServiceAsync}, che gestir\`a il ritorno dai metodi in maniera asincrona.

\subsubsection*{Relazioni d'uso di altre componenti}
Queste interfacce verranno implementate su \emph{server}, perci\`o non
avranno relazioni d'uso definite, se non con gli oggetti DTO (UserDTO,
...
UserDTO, UserSummaryDTO e UserCompleteDTO.
...
businness logic del sistema ovvero tutte quelle operazioni che potranno essere
compiute sui dati raccolti. Contiene inoltre delle classi di interfacciamento
con il databse stesso che consentono il controllo sulla persistenza dei dati.
...
\caption{Diagramma delle classi del package \emph{shared}.}
\end{figure}
\subsection*{Tipo, obiettivo e funzione del componente}
