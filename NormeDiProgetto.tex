...
\section{Norme per la Definizione di Prodotto}
Nella stesura del documento \emph{DefinizioneDiProdotto-\versioneDP.pdf} \`e
stata adottata la seguente notazione UML per le operazioni e gli attributi
dei diagrammi delle classi :
\begin{itemize}
  \item in generale visibilit\`a (+,-,\#) mentre in UiBinder @UiFiled e
  @UiHandler;
  \item nome del metodo/attributo;
  \item parametri specificati (esempio: in/out);
  \item ``:'' per il tipo di ritorno se presente;
  \item spazio;
  \item lista di altri tag (final, static, ecc.).
\end{itemize}
