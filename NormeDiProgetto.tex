...
\newcommand{\versioneNP}{3.0}
\newcommand{\versioneGL}{3.0}
\newcommand{\versionePP}{3.0}
\newcommand{\versionePQ}{3.0}
...
\begin{itemize}
  \item \bo{comprensione del dominio}, gli analisti partendo dall'enunciazione
  ...
  applicativo;
  \item \bo{raccolta dei requisiti}, interagire con i contraenti per
  identificare i loro requisiti;
  \item \bo{classificazione}, prendere la collezione non strutturata dei
  ...
  \item \bo{risoluzione dei conflitti}, nel caso in cui sorgano conflitti nei
  requisiti, bisogna individuarli e risolverli;
  \item \bo{attribuzione delle priorit\`a}, i requisiti vanno divisi ed
  etichettati (obbligatorio, desiderabile, opzionale) in base alla diversa
  importanza;
  \item \bo{validazione dei requisiti}, controllare i requisiti per capire se
  sono completi, consistenti e in accordo con ci\`o che i contraenti vogliono
  dal sistema.
\end{itemize}
Per semplificare la determinazione dei requisiti, si pu\`o decomporre l'uso
...
\subsection{Tracciamento dei requisiti - componenti}
...
\\
\bo{C(pc)(sc)n}\\
\bo{C} = (C = componente di persistenza e visualizzazione, C2 = componente di
recupero delle informazioni)\\ \bo{pc} = due lettere per identificare il
package\\ \bo{sc} = due lettere per identificare il sotto-package\\
...
\begin{itemize}
  ...
  \item \bo{C2ap1} = la classe DeviceScanner del package applet
\end{itemize}
...
\subsection{Norme per la Definizione del Prodotto}
...
\subsection{Convenzioni per i files}
...
\subsection{Convenzioni generali per i programmatori}
...
\begin{itemize}
  ...
  \hspace*{0.5cm} \co{//} \\
  \hspace*{0.5cm} \co{// Data di creazione} \\
  \hspace*{0.5cm} \co{//} \\
  \hspace*{0.5cm} \co{// Registro delle modifiche} \\
  \hspace*{0.5cm} \co{//} \\
  ;
...
\begin{itemize}
  ...
  (esempio: \co{runFast();});
  \item \bo{Variabili}: i nomi delle variabili non costanti devono essere scritti in
  inglese, tutti in minuscolo e nel caso in cui sono composti, le parole interne
  devono essere separate da underscore ``\_'' (esempio: \co{String
  example\_of\_variable;});
...
\section{Norme per i Test software}
...
\bo{n} = numero incrementale che indica la classe contenuta nel package
\\ \\
Esempio:
\begin{itemize}
  \item \bo{TU-Cclap3} = test d'unit\`a per la classe TranslateDTOXML del
