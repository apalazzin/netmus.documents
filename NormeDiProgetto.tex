...
18/01/2011 & 1.2 & Lovato Daniele & Aggiunto e redatto capitolo Norme per le
fasi di Analisi,Progettazione e Codifica\\
\hline
...
\item {RPP: Revisione del Progetto Preliminare; }
\item {RPD: Revisione del Progetto Definitivo;}
...
riportati i dettagli realtivi ad ogni strumento.
...
\chapter{Norme per le fasi di Analisi, Progettazione e Codifica}
\thispagestyle{fancy} 
\section{Norme per l'Analisi dei Requisiti}
Nell'analisi dei requisiti \`e stato utilizzato il linguaggio UML per la
modellazione delgli Use Case. \\
UML \`e stato scelto poich\`e adottato come standard per la scrittura dei Casi
D'uso.\\ La tecnologia utilizzata per la scrittura in UML \`e citata chiaramente
nel capitolo 4.7.\\
L'individuazione e il tracciamento dei requisiti \`e stata effettuata
analizzando a fondo i requisiti esposti nel capitolato d'appalto, e integrati
poi con i requisiti emersi dall'incontro effettuato con il proponente
trascritto nel documento Verbale-1.0.pdf.\\ Ricercando e testando programmi concorrenti simili a NetMus presenti nella rete globale, quali GrooveShark e LastFM, sono stati focalizzati nuovi requisiti che migliorano o integrano alcune funzioni di questi, così da redarre un'analisi
 dei requisiti pi\`u dettagliata.\\
  L'Analisi dei Requisiti \`e stata svolta tenendo presenti le esigenze di un qualunque utente che possa usufruire di NetMus.

\section{Norme per la Progettazione} 

\section{Norme per la Codifica}
Per la fase finale dello sviluppo del progetto, ossia per la codifica, sono state scelte tecnologie quali 
Eclipse per la programmazione Java con plugin di GAE per il Cloud Computing, per
la componente di Visualizzazione del Catalogo Multimediale.\\ Eclipse \`e stato
scelto come editor poichè tra i migliori nella rete per la programmazione Java,
oltre ad essere gi\`a stato ampiamente utilizzato da ognuno dei membri nel corso
di studi.\\ Inoltre eclipse offre plugin per GAE e GWT, e quindi offre un'unica
interfaccia per le tecnologie sfruttate.\\ Google App Engine \`e il più rinomato
software per il cloud Computing, e come scritto nello studio di Fattibilit\`a
desta interesse per tutti i membri del gruppo, con ore di formazione gratuite
fornite da uno dei proponenti del capitolato.\\
Usare come riferimento la sezione 4.1  per queste tecnologie.\\
Per la codifica della componente di Estrazione delle informazioni \`e stata
scelta la tecnologia JavaFX.\\
Questa tecnologia \`e stata suggerita dal proponente nel capitolato d'appalto, e
tra i vari test effettuati con altre tecnologie per lo sviluppo di Rich Internet Application \`e stato concordato come JavaFX sia la tecnologia migliore per lo sviluppo di questa componente.
...
un progetto di GanttProject appositamente condiviso e accessibile da ognuno
