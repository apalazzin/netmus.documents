...
\newcommand{\datamodifica}{28 Febbraio 2011}
...
dell'attività non ancora svolte, i test ed il collaudo interno.
...
Il Budget speso in questa fase ha combaciato quasi perfettamente con quello
preventivato. Infatti \`e stato consumato il 45.8\% del budget in questa fase
...
in meno rispetto a quanto preventivato. Le attivit\`a che hanno richiesti pi\`u
ore di lavoro sono state i test di qualifica e la codifica per l'ultima fase di
incremento. Per le restanti attivit\`a, le ore preventivate erano state
sovrastimate, infatti abbiamo un netto risparmio nelle ore per la stesura del
Manuale Utente definitivo e per la sua traduzione in inglese. In questa fase
l'attivit\`a di Verifica insieme ai test di qualifica sono state le attivit\`a
che hanno richiesto maggior dispendio di energie, hanno infatti occupato il
43.6\% dele ore totali di lavoro. Abbiamo infatti occupato le ore rimanenti che
il preventivo ci offriva per svolgere un'accurata verifica dei documenti e il
...
sforato con le ore di lavoro \`e Daminato che ha recuperato alcune ore di lavoro
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=11cm, angle=0]{img/PP/BUDGET-RA.png}
\caption{Stato del budget utilizzato fino a RA}
\end{figure}
\vspace{0.5cm}


In questa fase, il budget utilizzato ha rispettato il limite imposto dal
preventio parziale. Era infatti stata stimata una spesa di 2.195 Euro, il gruppo
ha invece speso solo 2.024 Euro, che corrisponde al 15\% del budget totale. Nel
...
lavoro e conseguentemente la porzione pi\`u sostanziosa di budget. A seguire i
programmatori hanno ultimato il loro lavoro, utilizzando anch'essi una buona
fetta del budget, seguiti dai ruoli di responsabile e amministratore, per finire
con i progettisti che hanno speso una minima quanit\`a di liquidi per rivedere
alcune scelte progettistiche a seguito della correzione della revisione di
qualifica.
...
----------tabella prev-cons personale per ruolo---------------
...
Il grafico soprastante mostra come sono state ripartita in percentuale le ore
...
altre attivit\`a. Ricordiamo che l'attivit\`a di analista \`e quasi assente
perch\`e le ore di analisi precedenti alla RR non sono state contabilizzate, e
quindi le ore visualizzate sono solamente quelle riguardanti l'analisi a seguito
della correzione RR e delle revisioni successive.
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=11cm, angle=0]{img/PP/BUDGET-FINE.png}
\caption{Stato del budget utilizzato a fine progetto}
\end{figure}
\vspace{0.5cm}

Come si pu\`o notare, non si \`e superata la soglia del budget, anzi si \`e
arrivati ad un risparmio di 197 Euro. Per comprendere meglio le spese, esponiamo
...
l'attivit\`a di verifica sia stata la più dispendiosa in termini di tempo, in
termini di costi la più dispendiosa \`e stata sicuramente quella di progettista,
influenzata anche dall'ampio lavoro di progettazione svolto dal gruppo. Seguono
come costi quello per il lavoro del verificatore e del programmatore.
