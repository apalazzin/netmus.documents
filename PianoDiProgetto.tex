...
Si pu\`o chiaramente vedere che la fase di verifica sar\`a presente
...
ripartizioni tra carico di lavoro e responsabilità.\\
...
all'analisi. L'analisi infatti non \`e stata contabilizzata nelle varie fasi di
lavoro poichè il prospetto economico parte dalla Revisione dei Requisiti(10
Gennaio) ossia quando la gran parte dell'analisi \`e stata gi\`a sviluppata.
L'analisi che viene riportata in questo grafico sono dei rivisitamenti dovuti
alla necessarie variazioni da effettuare decise alla RR.\\ 
...
La disponibilit\`a dei membri \`e perciò uno dei rischi maggiori. 
...
perciò almeno inizialmente questo sistema potrebbe risultare problematico e
creare ritardi nello sviluppo.\\
\\ 
\`E stato concordato che ogni membro potr\`a seguire un corso proposto
  dall'azienda di un altro capitolato, quindi le basi per l'utilizzo di questo sistema saranno assicurate per tutti. Il fattore di rischio perciò \`e dettato principalmente dalla difficolt\`a del sistema in s\`e.
...
L'unico inconveniente può essere la scrittura del codice del database che sar\`a implentato in GQL, linguaggio simile a SQL 
...
utilizzato nella rete. Ciò potrebbe ridurre
...
innovative funzionalit\`a, così da riuscire ad attirare un maggior numero di clienti.
...
lavoro, e in caso di problemi più complessi \`e invitato a comunicare il fatto al responsabile che cercher\`a una soluzione idonea.
...
complete, anzi una buona parte di questi file non contiene affatto informazioni, perciò la libreria virtuale 
...
funzione di ricerca automatica nel server dai record di tutti i brani registrati, così da completare 
